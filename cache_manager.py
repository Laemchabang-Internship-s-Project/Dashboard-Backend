...
OPD_TOTAL_ROOMS = (
    ...
    '108', '132', '069', '020', '019', '048', '901', '902', '903', '905'
)
...
def fetch_hos_sync():
    ...
    try:
        with SessionHOS() as db_hos:
            ...
            dept_selects = []
            for dept in TRACKED_DEPTS:
                # ยกเว้นห้อง 011 ER ไม่ต้องตรวจเช็คเวลาสิ้นสุด 16:00
                time_cond = "" if dept == "011" else "AND (s.service7 IS NULL OR s.service7 <= '16:00:00')"
                time_cond_total = "" if dept == "011" else "AND s.service7 <= '16:00:00'"
                
                dept_selects.append(f"""
                    ROUND(AVG(CASE WHEN o.main_dep='{dept}' AND s.service4 > s.service3 AND s.service7  IS NOT NULL {time_cond_total} THEN GREATEST((TIME_TO_SEC(s.service7) -TIME_TO_SEC(s.service3))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN o.main_dep='{dept}' AND s.service4 > s.service3 AND s.service4  IS NOT NULL {time_cond} THEN GREATEST((TIME_TO_SEC(s.service4) -TIME_TO_SEC(s.service3))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN o.main_dep='{dept}' AND s.service4 > s.service3 AND s.service5  IS NOT NULL AND s.service11 IS NOT NULL {time_cond} THEN GREATEST((TIME_TO_SEC(s.service5)-TIME_TO_SEC(s.service11))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN o.main_dep='{dept}' AND s.service4 > s.service3 AND s.service16 IS NOT NULL {time_cond} THEN GREATEST((TIME_TO_SEC(s.service16)-TIME_TO_SEC(IFNULL(s.service6,s.service12)))/60.0,0) END),1),
            ...
            svc_res = db_hos.execute(text(f"""
                SELECT
                    ROUND(AVG(CASE WHEN s.service4 > s.service3 AND s.service7  IS NOT NULL AND (s.service7 <= '16:00:00' OR o.main_dep = '011') THEN GREATEST((TIME_TO_SEC(s.service7) -TIME_TO_SEC(s.service3))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN s.service4 > s.service3 AND s.service4  IS NOT NULL AND (s.service7 IS NULL OR s.service7 <= '16:00:00' OR o.main_dep = '011') THEN GREATEST((TIME_TO_SEC(s.service4) -TIME_TO_SEC(s.service3))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN s.service4 > s.service3 AND s.service5  IS NOT NULL AND s.service11 IS NOT NULL AND (s.service7 IS NULL OR s.service7 <= '16:00:00' OR o.main_dep = '011') THEN GREATEST((TIME_TO_SEC(s.service5)-TIME_TO_SEC(s.service11))/60.0,0) END),1),
                    ROUND(AVG(CASE WHEN s.service4 > s.service3 AND s.service16 IS NOT NULL AND (s.service7 IS NULL OR s.service7 <= '16:00:00' OR o.main_dep = '011') THEN GREATEST((TIME_TO_SEC(s.service16)-TIME_TO_SEC(IFNULL(s.service6,s.service12)))/60.0,0) END),1),
